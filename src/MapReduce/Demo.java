	            ...
	            String[] tuple = value.toString().split("\\n");
	             for(int i=0;i<tuple.length; i++)
	             {
	            	 ...
	            	 if(bus.reviewCount != null && Integer.parseInt(bus.reviewCount) > 3)
	            	 {
	            		 ...
	            		 info.SetId(bus.businessId);
	            		 if(!(bus.category.isEmpty()) && bus.category.contains("Restaurants")){
	            			 if(info!= null && bus.city!= null){
	            				 context.write(new Text(bus.businessId), new Text("\t" + bus.name + "\t" + bus.category + "\t" +bus.state + "\t" + bus.city + "\t" + bus.latitude + "\t" + bus.longitude + "\t" + bus.reviewCount + "\t" + bus.stars));
	            			 }
	            		 }
	            	 }
	             }
